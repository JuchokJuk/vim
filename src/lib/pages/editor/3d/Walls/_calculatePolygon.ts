...
export function calculatePolygon(wallId: string, walls: Record<string, Wall>) {
	...
	endings.forEach((ending) => {
		Object.keys(wall[ending].connections).forEach((connectionId) => {
			const endings: Ending[] = ['start', 'end'];

			if (wall[ending].connections[connectionId] === 'end') {
			...
			wallConnectionsScheme[ending].all.push({
				id: connectionId,
				start: {
					x: walls[connectionId][endings[0]].x - wall[ending].x,
					y: walls[connectionId][endings[0]].y - wall[ending].y
				},
				end: {
					x: walls[connectionId][endings[1]].x - wall[ending].x,
					y: walls[connectionId][endings[1]].y - wall[ending].y
				}
			});
		});
	});
