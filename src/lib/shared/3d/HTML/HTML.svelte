<script>
	...
	import { useHasEventListeners } from './useHasEventListeners';
	import {
		compileStyles,
	...
	const getCamera = () => {
		if (!isViableCamera($camera)) {
			throw new Error(
				'Only PerspectiveCamera or OrthographicCamera supported for component <HTML>'
			);
		}
		return $camera;
	};
		...
		async () => {
			...
			if (
				...
				if (transform) {
					...
					const scale =
						distanceFactor === undefined ? 1 : objectScale(ref, camera) * distanceFactor;
